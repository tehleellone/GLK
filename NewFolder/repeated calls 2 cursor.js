// ============================================================
// repeated-calls.js — Repeated Calls Module v1.8.0
...
window.RC_MODULE_VERSION = '1.8.0';
...
function rcUniqueRepeatStatusCounts(items) {
    var repeat = (items || []).filter(function (it) { return rcIsRepeatMsisdn(it.MSISDN); });
    var pending = rcDedupeUniqueCallers(repeat.filter(function (it) {
        return rcDisplayStatus(it) === RC_STATUS.PENDING;
    })).filter(function (it) {
        return !rcMsisdnHasInProgressAssignment(it.MSISDN);
    });
    var inprogress = rcDedupeUniqueCallers(repeat.filter(function (it) {
        return rcDisplayStatus(it) === RC_STATUS.INPROGRESS;
    }));
    var resolved = rcDedupeUniqueCallers(repeat.filter(function (it) {
...
function rcDashboardMainHTML(dateFiltered, items, s) {
    return '<div class="top-stats">' +
            ...
            rcTile('Pending', s.pending, 'Unique repeat callers · not assigned', rcStatusColor(RC_STATUS.PENDING)) +
            rcTile('In Progress', s.inprogress, 'Unique repeat callers · assigned', rcStatusColor(RC_STATUS.INPROGRESS)) +
            rcTile('Resolved', s.completed, 'Unique repeat callers · done', rcStatusColor(RC_STATUS.RESOLVED)) +
        '</div>' +
...
function rcPdfAddChart(pdf, chartKey, title, y, margin, contentW) {
...
function rcEnsureReportCharts(items, summary) {
    ...
    if (section && wasHidden) section.style.display = 'grid';
...
window.rcExportPpt = async function () {
    ...
    try {
        ...
        s1.addText('Generated ' + new Date().toLocaleString('en-GB') + ' · ' + items.length + ' records', { x: 0.65, y: 4.65, w: 8.7, h: 0.3, fontSize: 10, color: '94A3B8', fontFace: 'Arial' });
        ...
        [
            ...
            s2.addText(row[0], { x: 0.55, y: 1.35 + i * 0.62, w: 2.4, h: 0.28, fontSize: 11, bold: true, color: C.deep, fontFace: 'Arial' });
            s2.addText(row[1], { x: 3.1, y: 1.35 + i * 0.62, w: 1.2, h: 0.28, fontSize: 14, bold: true, color: C.blue, fontFace: 'Arial' });
            s2.addText(row[2], { x: 4.4, y: 1.35 + i * 0.62, w: 5.1, h: 0.28, fontSize: 9.5, color: C.muted, fontFace: 'Arial' });
        });

        var s3 = pptx.addSlide();
        rcPptSlideHeader(s3, 'Activity Trend', 'Records over time · ' + report.dateLabel, C);
        s3.addChart(pptx.ChartType.line, [
            { name: 'Activities', labels: trend.labels, values: trend.values }
        ], { x: 0.35, y: 1.35, w: 9.3, h: 4.0, showLegend: false, showValue: false, lineSize: 3, chartColors: [C.blue], valGridLine: { style: 'none' } });

        var s4 = pptx.addSlide();
        rcPptSlideHeader(s4, 'Status Breakdown', 'Pending / in progress / resolved', C);
        ...
        ], { x: 2.2, y: 1.35, w: 5.6, h: 4.0, showLegend: true, legendPos: 'b', chartColors: [C.slate, C.amber, C.green] });

        var s5 = pptx.addSlide();
        rcPptSlideHeader(s5, 'SLA Distribution', 'Days assign → done/now', C);
        s5.addChart(pptx.ChartType.bar, [
            { name: 'Activities', labels: sla.labels, values: sla.values }
        ], { x: 0.35, y: 1.35, w: 9.3, h: 4.0, showLegend: false, showValue: true, barDir: 'col', chartColors: [C.green], valGridLine: { style: 'none' }, catGridLine: { style: 'none' } });

        var s6 = pptx.addSlide();
        rcPptSlideHeader(s6, 'Call Drivers', 'Count per driver', C);
        s6.addChart(pptx.ChartType.bar, [
            { name: 'Count', labels: drivers.labels, values: drivers.values }
        ], { x: 0.35, y: 1.35, w: 9.3, h: 4.0, showLegend: false, showValue: true, barDir: 'col', chartColors: [C.blue], valGridLine: { style: 'none' }, catGridLine: { style: 'none' }, catAxisLabelRotate: drivers.labels.length > 6 ? 35 : 0 });
...
window.rcExportPdf = async function () {
    ...
    try {
        ...
        y = rcPdfHeading(pdf, 'Repeated Calls Report', y, margin, 16);
        y = rcPdfLine(pdf, report.dateLabel, y, margin, contentW, 10);
        y = rcPdfLine(pdf, 'Generated: ' + new Date().toLocaleString('en-GB') + ' · Records: ' + items.length, y, margin, contentW, 10);
        ...
        pdf.addPage();
        y = margin;
        y = rcPdfHeading(pdf, 'Charts', y, margin, 14);
        y = rcPdfLine(pdf, 'Agent Workload is shown on the dashboard only and is excluded from this report.', y, margin, contentW, 9);
        y = rcPdfAddChart(pdf, 'trend', 'Activity Trend', y, margin, contentW);
        y = rcPdfAddChart(pdf, 'status', 'Status Breakdown', y, margin, contentW * 0.72);
        if (y > pdf.internal.pageSize.getHeight() - 70) { pdf.addPage(); y = margin; }
        y = rcPdfAddChart(pdf, 'drivers', 'Call Drivers', y, margin, contentW);
        if (y > pdf.internal.pageSize.getHeight() - 70) { pdf.addPage(); y = margin; }
        y = rcPdfAddChart(pdf, 'aging', 'SLA Distribution', y, margin, contentW);
