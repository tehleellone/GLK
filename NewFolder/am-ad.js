...
(function() {
    function inject() {
        ...
        <div id="amContent" style="display:none;">
            <!-- 4 Stats Tiles -->
            <div class="top-stats" style="grid-template-columns: repeat(4, 1fr);">
                <div class="stat-card">
...
<div class="dashboard" style="display:none;" id="createRequestView">
    ...
    if (document.readyState === 'loading') {
        ...
        async function initAMView() {
            try {
                ...
                if (userRole === 'Account Manager') {
                    ...
                    AM_DATA = window.AM_ALL_DATA.filter(a => a.type === 'Group');
        ...
        function renderAMTable() {
            ...
            window.AM_PARENT_ROWS = parentRows;
            ...
            const columnDefs = [{
                ...
                },
             {
                ...
                },
                {
                    ...
                    width: 150,
                    cellRenderer: params => {
                        const badge = (params.value === 'OnBoarded' || params.value === 'AM_Approved') ? 'badge-success' : 'badge-warning';
                        return `<span class="status-badge ${badge}">${params.value}</span>`;
                    }
                }
            ];
        ...
        function populateRequestDropdowns() {
            ...
            // Set Account Director (readonly, prepopulated)
            const adInput = document.getElementById('reqAD');

            if (currentUserRole === 'Account Director') {
                adInput.value = currentUserName;
                console.log('[✓] AD set to:', currentUserName);
            } else if (currentUserRole === 'Account Manager') {
                // For AM, find their AD from their accounts
                const myAccount = AM_DATA.find(a => a.am === currentUserName && a.ad);
                if (myAccount) {
                    adInput.value = myAccount.ad;
                    console.log('[✓] AD found from account:', myAccount.ad);
                } else {
                    // Fallback: Try to find any account with this AM
                    const anyAccount = ALL_DATA.find(a => a.am === currentUserName && a.ad);
                    if (anyAccount) {
                        adInput.value = anyAccount.ad;
                        console.log('[✓] AD found from ALL_DATA:', anyAccount.ad);
                    } else {
                        console.error('[✗] No AD found for AM:', currentUserName);
                        adInput.value = '';
                    }
                }
            }
    ...
    `).join('');
        ...
        function reviewCreateRequest() {
            ...
            const ad = document.getElementById('reqAD').value.trim();
        ...
        async function submitRequest() {
            ...
            try {
                ...
                // Get people IDs
                const adName = document.getElementById('reqAD').value.trim();
...
Account Director: ${document.getElementById('reqAD').value}
