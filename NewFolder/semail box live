/* ═══════════════════════════════════════════════════════════════
   SEMAILBOX-LIVE.JS — SE Mailbox Intelligence v7.0
   Live Outlook via Microsoft Graph (no Excel upload)
   BA-grade: SLA · Agent drilldown · Forecasting · Insights
   Rules: Unassigned = no Agent/Citrix · Assigned = Agent/Citrix
          Completed = Flag date · Age based on received date
   ═══════════════════════════════════════════════════════════════ */
'use strict';

/* Early global stub — prevents "initSEMailbox is not defined" if script fails mid-load */
(function(w) {
    if (!w) return;
    w.__SEMAILBOX_VERSION = '7.0-live';
    w.initSEMailbox = function() {
        if (w.SEMailbox && typeof w.SEMailbox.init === 'function') return w.SEMailbox.init();
        console.error('[SEMailbox] Script did not finish loading. Re-upload semailbox.js to SharePoint Shared Documents and hard-refresh (Ctrl+F5).');
        var root = document.getElementById('semailbox-root');
        if (root) root.innerHTML = '<div style="padding:20px;color:#e74c3c;font-weight:700;">SE Mailbox script failed to load.<br><span style="font-size:13px;font-weight:500;">Re-upload <b>semailbox.js</b> to SharePoint Shared Documents, then press Ctrl+F5.</span></div>';
    };
})(typeof window !== 'undefined' ? window : null);

function mbSafeStorageGet(key) {
    try { return localStorage.getItem(key) || ''; } catch (e) { return ''; }
}
function mbSafeStorageSet(key, val) {
    try { localStorage.setItem(key, val); } catch (e) {}
}

/* ── CONFIG ──────────────────────────────────────────────────── */
const MB_CONFIG = {
    SP_SITE: (typeof _spPageContextInfo !== 'undefined' && _spPageContextInfo && _spPageContextInfo.webAbsoluteUrl)
        ? _spPageContextInfo.webAbsoluteUrl
        : ((typeof window !== 'undefined' && window.location && window.location.origin)
            ? window.location.origin : '') + '/sites/SM',
    ACCT_MAP_LIST: 'Account Mapping',

    /* ── Microsoft Graph / Outlook (EDIT BEFORE DEPLOY) ── */
    MAILBOX_UPN: 'du.ServiceManagement@du.ae',
    GRAPH_SCOPE: 'https://graph.microsoft.com',
    MAX_MESSAGES: 5000,
    DAYS_BACK: 120,
    POLL_MS: 5 * 60 * 1000,

    /* Optional MSAL fallback if SharePoint token acquire fails — register app in Azure AD */
    MSAL_CLIENT_ID: '',
    MSAL_TENANT_ID: 'common',

    /* Set true to fall back to legacy Excel file on SharePoint (semailbox v6 behaviour) */
...
let MB = {
    ...
    filterLocations: [],   // [] = all
    filterSlaStatus: [],   // [] = all
    filterSlaTypes:  [],   // SLA column values — [] = all
    filterSenderTypes: [], // Sender Type column — [] = all
    filterYears:  [],
    filterMonths: [],
    filterWeeks:  [],
    filterDays:   [],
    includeBacklog: false, // when day filter active: include open items from earlier days
    ...
    trendPeriod:     'W',
    drillAgent:      null,
    activeSourceFilter: null,  // sender type key (lowercase) or 'repeat'
    showRepeatThreads: false,
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
function mbResolveStatus(tabName, flagDate, agentName, citrixId, assignmentStatus, caseStatus) {
    const asgL = String(assignmentStatus || '').toLowerCase().trim();
    const caseL = String(caseStatus || '').toLowerCase().trim();
    const hasFlag = mbHasValue(flagDate);
    const hasAgent = mbHasValue(agentName) || mbHasValue(citrixId);

    // Excel Raw Data columns take precedence when present
    if (asgL || caseL) {
        if (hasFlag || caseL === 'closed') return 'Completed';
        if (asgL === 'unassigned') return 'Unassigned';
        if (asgL === 'assigned') return 'In Progress';
    }
...
function mbComputeAgeDays(status, received) {
    if (status === 'Completed' || status === 'Closed') return null;
    return mbDaysSince(received);
}
function mbComputeAgeHours(status, received, ageingHrs) {
    if (status === 'Completed' || status === 'Closed') return null;
    if (ageingHrs !== '' && ageingHrs != null && !isNaN(Number(ageingHrs))) return Number(ageingHrs);
    const days = mbComputeAgeDays(status, received);
    return days != null ? Math.round(days * 24 * 10) / 10 : null;
}
...
/* ══════════════════════════════════════════════════════════════
   SENDER TYPE & REPEAT THREAD HELPERS
   ══════════════════════════════════════════════════════════════ */
const MB_SENDER_ORDER = ['Internal Team', 'Customer', 'Channel Partner'];
const MB_SENDER_COLORS = {
    'internal team': '#4c6fff',
    'customer': '#27ae60',
    'channel partner': '#a855f7',
};
const MB_SENDER_ICONS = {
    'internal team': '🏢',
    'customer': '🌐',
    'channel partner': '🤝',
};
function mbNormSenderType(v) {
    const s = String(v ?? '').trim();
    return s || 'Unknown';
}
function mbSenderColor(type) {
    const k = mbNormSenderType(type).toLowerCase();
    if (MB_SENDER_COLORS[k]) return MB_SENDER_COLORS[k];
    let h = 0;
    for (let i = 0; i < k.length; i++) h = ((h << 5) - h) + k.charCodeAt(i);
    return MB_CLR.smColors[Math.abs(h) % MB_CLR.smColors.length];
}
function mbSenderIcon(type) {
    return MB_SENDER_ICONS[mbNormSenderType(type).toLowerCase()] || '📧';
}
function mbExtractEmail(from) {
    const m = String(from || '').match(/[\w.+-]+@[\w.-]+\.\w+/i);
    return m ? m[0].toLowerCase() : '';
}
function mbNormalizeSubject(subject) {
    let s = String(subject || '').trim();
    for (let i = 0; i < 12; i++) {
        const next = s.replace(/^(re|fw|fwd|aw|antw)\s*(\[\d+\])?\s*:\s*/i, '').trim();
        if (next === s) break;
        s = next;
    }
    return s.toLowerCase();
}
function mbEnrichTickets(tickets) {
    const subMap = {};
    tickets.forEach(t => {
        const key = t.subjectKey || mbNormalizeSubject(t.subject);
        t.subjectKey = key;
        t.senderType = mbNormSenderType(t.senderType);
        if (key) subMap[key] = (subMap[key] || 0) + 1;
    });
    tickets.forEach(t => {
        const cnt = t.subjectKey ? (subMap[t.subjectKey] || 1) : 1;
        t.subjectRepeatCount = cnt;
        t.isRepeatSubject = cnt > 1;
        if (!t.fromEmail) t.fromEmail = mbExtractEmail(t.from);
    });
    return tickets;
}
...
function mbParseSheet(sheet, tabName) {
    ...
    rows.forEach((row, idx) => {
        ...
        const from      = String(n['from']||n['to']||'').trim();
        const sender    = String(n['sender']||'').trim();
        ...
        const assignmentStatus = String(n['assignment status']||'').trim();
        const caseStatus = String(n['case status']||'').trim();
        const mailboxType = String(n['mailbox type']||'').trim();
        const pendingReason = String(n['pending reason']||n['pendingreason']||'').trim();
        const owner     = String(n['owner']||'').trim();
        const senderType = mbNormSenderType(n['sender type']||n['sendertype']||n['sender_type']||'');
        const fromDisplay = from || sender;
        ...
        const status    = mbResolveStatus(tabName, flagDate, agentName, citrixId, assignmentStatus, caseStatus);
        const ageDays   = mbComputeAgeDays(status, received);
        const ageHours  = mbComputeAgeHours(status, received, ageingHrs);
        const subjectKey = mbNormalizeSubject(subject);
        ...
        out.push({
            ...
            from: fromDisplay, sender, subject, subjectKey, senderType, fromEmail: mbExtractEmail(fromDisplay),
            received, receivedFmt: mbFmtDate(received),
            ...
            slaStatus, assignmentStatus, caseStatus, mailboxType, resolveDays,
            smEmail: mapped?.Email_ID||'', smTeam: mapped?.Team||location||'',
            status, ageDays, ageHours,
            owner, pendingReason,
            ...
            subjectRepeatCount: 1, isRepeatSubject: false,
...
const MB_SKIP_SHEETS = new Set([
    'master categories','active list','sheet1','sheet2','sheet3',
    'email handling','day on day inflow','week on week inflow','channel partners',
]);
...
function mbProcessWorkbook(wb) {
    ...
    mbApplyTicketDataset(all, sent, hasSent);
}

function mbApplyTicketDataset(all, sent, hasSent) {
    MB.allTickets   = mbEnrichTickets(all);
    MB.sentTickets  = sent;
    ...
    MB.ageFilter    = null;
    MB.drillAgent   = null;
    MB.activeSourceFilter = null;
    MB.filterSMs    = [];
    MB.filterLocations = [];
    MB.filterSlaStatus = [];
    MB.filterSlaTypes  = [];
    MB.filterSenderTypes = [];
    MB.filterYears  = [];
    MB.filterMonths = [];
    MB.filterWeeks  = [];
    MB.filterDays   = [];
    MB.includeBacklog = false;
    MB.dataLoaded   = true;
...
function mbTicketFromFields(row, tabName, idx) {
    const from      = String(row.from || row.sender || '').trim();
    const sender    = String(row.sender || '').trim();
    const subject   = String(row.subject || row.title || '');
    const received  = row.received || row.receivedDateTime || '';
    const flagDate  = row.flagDate || row.completedDate || '';
    const catRaw    = String(row.categories || row.catRaw || '').trim();
    const parsedCat = String(row.category || '').trim();
    const agentName = String(row.agentName || row.agent || '').trim();
    let citrixId    = String(row.citrixId || row.citrix || '').trim();
    const location  = String(row.location || '').trim();
    const sla       = row.sla ?? '';
    const slaDue    = row.slaDue || '';
    const ageing    = String(row.ageing || '').trim();
    const ageingHrs = row.ageingHours;
    const slaStatus = String(row.slaStatus || '').trim();
    const assignmentStatus = String(row.assignmentStatus || '').trim();
    const caseStatus = String(row.caseStatus || '').trim();
    const mailboxType = String(row.mailboxType || '').trim();
    const pendingReason = String(row.pendingReason || '').trim();
    const owner     = String(row.owner || '').trim();
    const senderType = mbNormSenderType(row.senderType || '');
    const fromDisplay = from || sender;
    ...
    return {
        id: row.id || `${tabName}-${idx}`,
        tab: tabName,
        graphId: row.graphId || '',
        from: fromDisplay, sender, subject, subjectKey, senderType,
        fromEmail: mbExtractEmail(fromDisplay),
        received, receivedFmt: mbFmtDate(received),
        flagDate, flagDateFmt: mbFmtDate(flagDate),
        catRaw, category: issueCategory, smCode: citrixId, smName: agentName || parsed.smName,
        smFullName, agentName, citrixId, location,
        sla, slaDue, slaDueFmt: mbFmtDate(slaDue),
        ageing, ageingHours: ageingHrs !== '' && ageingHrs != null && !isNaN(Number(ageingHrs)) ? Number(ageingHrs) : null,
        slaStatus, assignmentStatus, caseStatus, mailboxType, resolveDays,
        smEmail: mapped?.Email_ID || '', smTeam: mapped?.Team || location || '',
        status, ageDays, ageHours,
        owner, pendingReason,
...
/* ══════════════════════════════════════════════════════════════
   MICROSOFT GRAPH — LIVE OUTLOOK
   ══════════════════════════════════════════════════════════════ */
function mbGraphSinceISO() {
    const d = new Date();
    d.setDate(d.getDate() - (MB_CONFIG.DAYS_BACK || 120));
    return d.toISOString();
}

async function mbGetSpDigest() {
    const el = document.getElementById('__REQUESTDIGEST');
    if (el && el.value) return el.value;

    const site = MB_CONFIG.SP_SITE;
    const res = await fetch(`${site}/_api/contextinfo`, {
        method: 'POST',
        headers: { Accept: 'application/json;odata=verbose' },
        credentials: 'include',
    });
    if (!res.ok) throw new Error('Could not get SharePoint security token (contextinfo). Refresh the page.');
    const json = await res.json();
    const digest = json?.d?.GetContextWebInformation?.FormDigestValue;
    if (!digest) throw new Error('SharePoint FormDigest missing — reload SM Portal and try again.');
    return digest;
}

async function mbAcquireGraphToken() {
    const site = MB_CONFIG.SP_SITE;

    /* 1 — SharePoint OAuth bridge (needs X-RequestDigest on POST) */
    try {
        const digest = await mbGetSpDigest();
        const res = await fetch(`${site}/_api/SP.OAuth.Token/Acquire`, {
            method: 'POST',
            headers: {
            ...
            credentials: 'include',
            body: JSON.stringify({ resource: MB_CONFIG.GRAPH_SCOPE }),
        });
        const json = await res.json().catch(() => ({}));
        if (res.ok) {
            const tok = json?.d?.AccessToken || json?.d?.access_token;
            if (tok) return tok;
        }
        const spMsg = json?.error?.message?.value || json?.error?.message || '';
        if (spMsg) console.warn('[SEMailbox] SP.OAuth.Token/Acquire:', spMsg);
    } catch (e) {
        console.warn('[SEMailbox] SharePoint Graph token failed', e);
    }

    /* 2 — MSAL popup (set MSAL_CLIENT_ID in MB_CONFIG if IT provides an Azure app) */
    if (MB_CONFIG.MSAL_CLIENT_ID) {
        if (typeof msal === 'undefined') {
            await new Promise((resolve, reject) => {
                const s = document.createElement('script');
                s.src = 'https://alcdn.msauth.net/browser/2.38.3/js/msal-browser.min.js';
                s.onload = resolve;
                s.onerror = () => reject(new Error('Could not load MSAL library'));
                document.head.appendChild(s);
            });
        }
        const cfg = {
            auth: {
                clientId: MB_CONFIG.MSAL_CLIENT_ID,
                authority: `https://login.microsoftonline.com/${MB_CONFIG.MSAL_TENANT_ID || 'common'}`,
                redirectUri: window.location.origin,
            },
            cache: { cacheLocation: 'sessionStorage' },
        };
        const app = new msal.PublicClientApplication(cfg);
        await app.initialize();
        const scopes = ['Mail.Read.Shared', 'Mail.Read'];
        try {
            const accounts = app.getAllAccounts();
            if (accounts.length) {
                const silent = await app.acquireTokenSilent({ scopes, account: accounts[0] });
                if (silent?.accessToken) return silent.accessToken;
            }
        } catch (e) { /* popup next */ }
        const popup = await app.loginPopup({ scopes });
        return popup.accessToken;
    }

    throw new Error(
        'Graph token failed. SharePoint security token was rejected or Graph is not enabled. ' +
        'Ask IT for Mail.Read.Shared on du.ServiceManagement@du.ae — or set MSAL_CLIENT_ID for MSAL login.'
    );
}

async function mbGraphGet(token, url) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`Graph ${res.status}: ${errBody.slice(0, 280) || res.statusText}`);
    }
    return res.json();
}

async function mbGraphFetchFolderMessages(token, mailbox, folderId, tabName) {
    const since = mbGraphSinceISO();
    const select = [
        'id', 'subject', 'from', 'sender', 'receivedDateTime', 'sentDateTime',
        'categories', 'flag', 'isRead', 'bodyPreview', 'toRecipients',
        'lastModifiedDateTime', 'conversationId',
    ].join(',');
    const filter = encodeURIComponent(`receivedDateTime ge ${since}`);
    let url = `${MB_CONFIG.GRAPH_SCOPE}/v1.0/users/${encodeURIComponent(mailbox)}/mailFolders/${folderId}/messages?$select=${select}&$filter=${filter}&$orderby=receivedDateTime desc&$top=250`;
    const out = [];
    let guard = 0;

    while (url && out.length < (MB_CONFIG.MAX_MESSAGES || 5000) && guard < 40) {
        guard++;
        const page = await mbGraphGet(token, url);
        (page.value || []).forEach((msg, idx) => {
            out.push(mbGraphMessageToTicket(msg, tabName, out.length + idx));
        });
...
function mbCategoryMeta(categories) {
    const cats = (categories || []).map(c => String(c).trim()).filter(Boolean);
    const joined = cats.join(', ');
    let agentName = '', citrixId = '', location = '', senderType = '', slaStatus = '', assignmentStatus = '', caseStatus = '';
    let pendingReason = '', sla = '', owner = '';

    cats.forEach(c => {
        const cl = c.toLowerCase();
        if (/^cnfx/i.test(c)) {
            citrixId = mbExtractCitrix(c) || citrixId;
            const parsed = mbParseCategories(c);
            if (parsed.smName) agentName = agentName || parsed.smName;
        }
        if (cl.includes('within sla')) slaStatus = 'Within SLA';
        if (cl.includes('outside sla')) slaStatus = 'Outside SLA';
        if (cl === 'unassigned') assignmentStatus = 'Unassigned';
        if (cl === 'assigned') assignmentStatus = 'Assigned';
        if (cl === 'closed' || cl === 'completed') caseStatus = 'Closed';
        if (cl.startsWith('location:')) location = c.split(':').slice(1).join(':').trim();
        if (cl.startsWith('sender type:') || cl.startsWith('sendertype:')) senderType = c.split(':').slice(1).join(':').trim();
        if (cl.startsWith('pending:')) pendingReason = c.split(':').slice(1).join(':').trim();
        if (cl.startsWith('owner:')) owner = c.split(':').slice(1).join(':').trim();
        if (cl.startsWith('sla:')) sla = c.split(':').slice(1).join(':').trim();
    });

    const parsed = mbParseCategories(joined);
    if (!citrixId) citrixId = parsed.smCode;
    if (!agentName) agentName = parsed.smName;

    return {
        catRaw: joined,
        category: parsed.category,
        agentName, citrixId, location, senderType, slaStatus,
        assignmentStatus, caseStatus, pendingReason, sla, owner,
    };
}

function mbInferSenderType(fromEmail, metaType) {
    if (mbHasValue(metaType)) return metaType;
    const e = String(fromEmail || '').toLowerCase();
    if (!e) return 'Unknown';
    if (e.includes('partner') || e.includes('channel')) return 'Channel Partner';
    if (e.includes('@') && (e.includes('internal') || e.endsWith('.local'))) return 'Internal Team';
    return 'Customer';
}

function mbGraphMessageToTicket(msg, tabName, idx) {
    const fromObj = msg.from?.emailAddress || msg.sender?.emailAddress || {};
    const fromDisplay = fromObj.name
        ? `${fromObj.name} <${fromObj.address || ''}>`.trim()
        : (fromObj.address || '');
    const meta = mbCategoryMeta(msg.categories);
    const flagComplete = msg.flag?.flagStatus === 'complete' || msg.flag?.flagStatus === 'completed';
    const flagDate = msg.flag?.completedDateTime || (flagComplete ? msg.lastModifiedDateTime : '');
    const received = msg.receivedDateTime || msg.sentDateTime || '';
    const ageHrs = received ? Math.round((Date.now() - new Date(received).getTime()) / 3600000 * 10) / 10 : null;

    return mbTicketFromFields({
        graphId: msg.id,
        id: `graph-${tabName}-${msg.id || idx}`,
        from: fromDisplay,
        subject: msg.subject || '(No subject)',
        received,
        flagDate,
        categories: meta.catRaw,
        category: meta.category,
        agentName: meta.agentName,
        citrixId: meta.citrixId,
        location: meta.location,
        sla: meta.sla,
        slaStatus: meta.slaStatus || (ageHrs != null && ageHrs > 48 ? 'Outside SLA' : 'Within SLA'),
        assignmentStatus: meta.assignmentStatus,
        caseStatus: meta.caseStatus,
        pendingReason: meta.pendingReason,
        owner: meta.owner,
        senderType: mbInferSenderType(fromObj.address, meta.senderType),
        ageingHours: ageHrs,
        mailboxType: tabName === 'sent' ? 'Sent' : 'Inbox',
    }, tabName, idx);
}

async function mbLoadFromOutlook(showStatus) {
    if (MB.loading) return MB.dataLoaded;
    MB.loading = true;
    MB.graphError = null;
    const silent = showStatus === false;
    if (!silent) mbShowStatus('Loading live mailbox from Outlook…');

    try {
        const mailbox = (MB_CONFIG.MAILBOX_UPN || '').trim();
        if (!mailbox || mailbox.includes('yourcompany')) {
            throw new Error('Set MB_CONFIG.MAILBOX_UPN to your SE shared mailbox address (e.g. tsm-se@company.com).');
        }

        const token = await mbAcquireGraphToken();
        const inbox = await mbGraphFetchFolderMessages(token, mailbox, 'inbox', 'inbox');
        let sent = [];
        try {
            sent = await mbGraphFetchFolderMessages(token, mailbox, 'sentitems', 'sent');
        } catch (e) {
            console.warn('[SEMailbox] Sent folder skipped:', e.message);
        }

        MB.lastSyncISO = new Date().toISOString();
        MB.lastSyncFmt = mbFmtDate(MB.lastSyncISO);
        mbApplyTicketDataset(inbox, sent, sent.length > 0);
        mbShowStatus('');
        if (!silent) mbShowToast(`✅ Live Outlook sync — ${inbox.length} inbox, ${sent.length} sent`, 'success');
        return true;
    } catch (err) {
        console.error('[SEMailbox] Graph load failed:', err);
        MB.graphError = err.message || String(err);
        mbShowStatus('');
        mbUpdateLiveBar(true);

        if (MB_CONFIG.FALLBACK_EXCEL) {
            mbShowToast('Graph failed — trying Excel fallback…', 'warning');
            const ok = await mbLoadFromSP();
            if (ok) return true;
        }

        mbShowToast('❌ ' + MB.graphError, 'error');
        return false;
    } finally {
        MB.loading = false;
    }
...
function mbStartLivePoll() {
    if (MB.livePollTimer) return;
    MB.livePollTimer = setInterval(() => {
        if (document.hidden) return;
        const section = document.getElementById('seMailboxView');
        if (!section || section.style.display === 'none') return;
        mbLoadFromOutlook(false);
    }, MB_CONFIG.POLL_MS || 300000);
}

function mbStopLivePoll() {
    if (MB.livePollTimer) {
        clearInterval(MB.livePollTimer);
        MB.livePollTimer = null;
    }
}

function mbUpdateLiveBar(isError) {
    const bar = document.getElementById('mb-live-bar');
    if (!bar) return;
    const mailbox = MB_CONFIG.MAILBOX_UPN || '—';
    if (isError && MB.graphError) {
        bar.innerHTML = `<span style="color:#e74c3c;font-weight:600;">⚠ ${MB.graphError}</span>`;
        return;
    }
    const when = MB.lastSyncFmt || 'Not synced yet';
    const cnt = MB.dataLoaded ? `${MB.allTickets.length} emails` : '—';
    bar.innerHTML = `<span style="color:var(--t2);">📡 Live Outlook · <b>${mailbox}</b> · ${cnt} · Updated <b>${when}</b></span>`;
}

function mbRefreshLive() {
    return mbLoadFromOutlook(true);
}

/* ══════════════════════════════════════════════════════════════
   LEGACY EXCEL FALLBACK (optional — MB_CONFIG.FALLBACK_EXCEL)
   ══════════════════════════════════════════════════════════════ */
async function mbLoadFromSP() {
    try {
        mbShowStatus('Loading mailbox data from Excel…');
        ...
        if (!res.ok) throw new Error('Excel file not found on SharePoint');
...
function mbApplyFilters() {
    ...
    if (MB.filterSlaTypes.length > 0)
        data = data.filter(t => MB.filterSlaTypes.includes(String(t.sla || 'Unknown').trim() || 'Unknown'));
    if (MB.filterSenderTypes.length > 0)
        data = data.filter(t => MB.filterSenderTypes.includes(t.senderType || 'Unknown'));
    ...
    if (MB.filterDays.length > 0) {
        const minDay = MB.filterDays.slice().sort()[0];
        if (MB.includeBacklog) {
            data = data.filter(t =>
                MB.filterDays.includes(t.recvDay) ||
                (mbIsOpen(t) && t.recvDay && t.recvDay < minDay)
            );
        } else {
    ...
    if (MB.activeSourceFilter) {
        if (MB.activeSourceFilter === 'repeat') data = data.filter(t => t.isRepeatSubject);
        else data = data.filter(t => (t.senderType || 'Unknown').toLowerCase() === MB.activeSourceFilter);
    }
    ...
    if (MB.activeSlaFilter) {
        const fn = {
            within:  t => mbIsWithin(t) && mbIsOpen(t),
            outside: t => mbIsOutside(t) && mbIsOpen(t),
        }[MB.activeSlaFilter];
        if (fn) data = data.filter(fn);
    }
    ...
    if (MB.ageFilter) {
        ...
        const ageOk = t => {
            const hrs = t.ageHours;
            if (hrs === null || hrs === undefined) return false;
            if (MB.ageFilter === '0-24') return hrs >= 0 && hrs < 24;
            if (MB.ageFilter === '24-48') return hrs >= 24 && hrs < 48;
            return hrs >= 48;
        };
...
/* ══════════════════════════════════════════════════════════════
   KPI + METRIC ENGINE
   ══════════════════════════════════════════════════════════════ */
function mbIsOutside(t) {
    const s = (t.slaStatus||'').toLowerCase();
    return s.includes('outside') && !s.startsWith('closed');
}
function mbIsWithin(t)  {
    const s = (t.slaStatus||'').toLowerCase();
    return s.includes('within') && !s.startsWith('closed');
}
function mbIsOpen(t) {
    if (t.caseStatus) return String(t.caseStatus).toLowerCase() === 'open';
    return t.status==='In Progress' || t.status==='Unassigned';
}
...
function mbGetKPIs(t) {
    ...
    return {
        ...
        slaWithinPct:  slaTotal ? Math.round((within/slaTotal)*100) : null,
        slaOutsidePct: slaTotal ? Math.round((outside/slaTotal)*100) : null,
        avgAge:     avgAge,
...
/* Metric definitions — shown via (i) tooltips */
const MB_METRIC_TIPS = {
    within:    'Open emails (Unassigned or In Progress) where SLA Status is "Within SLA". Percentage = share of all open SLA-tracked emails.',
    outside:   'Open emails where SLA Status is "Outside SLA" — past due and still unresolved. Percentage = share of all open SLA-tracked emails.',
    compliance:'SLA Compliance = (Within SLA ÷ (Within + Outside)) × 100 for open emails only. Completed emails are excluded. Green ≥90%, Amber ≥70%, Red <70%.',
    dueSoon:   'Open emails whose SLA Due date falls within the next 24 hours. These need attention before they breach.',
    age024:    'Open emails aged 0–24 hours since received date.',
    age2448:   'Open emails aged 24–48 hours since received date.',
    age48p:    'Open emails aged 48 hours or more since received date.',
    backlog:   'When a day is selected: include still-open emails received before the earliest selected day. Turn off to see only emails received on the selected day(s).',
    senderType:'Sender Type from the Excel column — e.g. Internal Team, Customer, Channel Partner. Count & % reflect current filters.',
    repeat:    'Emails sharing the same subject thread (Re:/Fw: prefixes stripped). Count = emails in repeat threads; % = share of total.',
};
function mbInfoIcon(tip) {
    const safe = String(tip).replace(/"/g, '&quot;');
    return `<span title="${safe}" style="display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;border-radius:50%;background:var(--bg-secondary);border:1px solid var(--border);font-size:9px;font-weight:800;color:var(--t3);cursor:help;margin-left:4px;flex-shrink:0;line-height:1;" onclick="event.stopPropagation();">i</span>`;
}
...
function mbRenderTopFilters() {
    ...
    const allSlaTypes = [...new Set(MB.allTickets.map(t=>String(t.sla||'').trim()||'Unknown').filter(Boolean))].sort();
    const allSenderTypes = [...new Set(MB.allTickets.map(t=>t.senderType||'Unknown').filter(Boolean))].sort((a,b)=>{
        const ai = MB_SENDER_ORDER.indexOf(a), bi = MB_SENDER_ORDER.indexOf(b);
        ...
        return `<div style="flex:1;min-width:130px;">
            ...
            <div style="position:relative;">
                ...
                <div id="${id}-dd" style="display:none;position:absolute;top:calc(100% + 4px);left:0;min-width:180px;z-index:9999;background:var(--bg-card);border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.18);max-height:240px;overflow-y:auto;">
                    ...
                    ${opts.map(o=>{
                        const lbl = label==='Week' ? o : ((label==='Day'||label==='Received Day') ? new Date(o+'T00:00').toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}) : o);
    ...
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;padding:12px 16px;background:var(--bg-card);border:1px solid var(--border);border-radius:12px;box-shadow:var(--cs);margin-bottom:1rem;">
        ...
        ${dd('mb-f-slat', 'SLA Type',   allSlaTypes, MB.filterSlaTypes, 'mbFilterSlaType')}
        ${dd('mb-f-stype','Sender Type', allSenderTypes, MB.filterSenderTypes, 'mbFilterSenderType')}
        ...
        ${dd('mb-f-day',  'Received Day', allDays, MB.filterDays,      'mbFilterDay')}
        ${MB.filterDays.length > 0 ? `
        <div style="flex:0 0 auto;min-width:160px;align-self:flex-end;">
            <div style="font-size:.62rem;font-weight:700;text-transform:uppercase;color:var(--t3);letter-spacing:.05em;margin-bottom:4px;display:flex;align-items:center;">Backlog ${mbInfoIcon(MB_METRIC_TIPS.backlog)}</div>
            <select onchange="MB.includeBacklog=this.value==='yes';mbApplyFilters();mbRenderAll();" style="width:100%;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--bg-input);font-size:12px;color:var(--t1);cursor:pointer;">
                <option value="no" ${!MB.includeBacklog?'selected':''}>Received day only</option>
                <option value="yes" ${MB.includeBacklog?'selected':''}>Include prior backlog</option>
            </select>
        </div>` : ''}
...
window.mbFilterSlaType = (v,cb) => _mbSetFilter(MB.filterSlaTypes, v, cb);
window.mbFilterSenderType = (v,cb) => _mbSetFilter(MB.filterSenderTypes, v, cb);
...
window.mbClearAllFilters = () => {
    MB.filterSMs=[]; MB.filterLocations=[]; MB.filterSlaStatus=[]; MB.filterSlaTypes=[]; MB.filterSenderTypes=[];
    MB.filterYears=[]; MB.filterMonths=[]; MB.filterWeeks=[]; MB.filterDays=[];
    MB.includeBacklog=false;
    MB.activeKpiFilter=null; MB.activeSlaFilter=null; MB.ageFilter=null; MB.drillAgent=null; MB.activeSourceFilter=null;
    mbApplyFilters(); mbRenderAll();
};
...
function mbRenderSlaPanel() {
    ...
    <div class="table-section" style="margin-bottom:1rem;">
        <div class="table-header" style="margin-bottom:.75rem;">
            <h3 class="table-title"><i data-lucide="shield-check" style="width:18px;height:18px;display:inline-block;vertical-align:middle;margin-right:6px;"></i>SLA Performance Overview</h3>
            <div style="font-size:12px;color:var(--t3);">Open emails only${MB.filterSlaTypes.length?' · filtered by SLA type':''}</div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.75rem;">
            <div onclick="mbToggleSla('within')" style="cursor:pointer;background:var(--bg-card);border:2px solid ${sf==='within'?'#27ae60':'var(--border)'};border-radius:12px;padding:1rem;${sf==='within'?'background:#27ae6018;':''}box-shadow:var(--cs);">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:var(--t3);margin-bottom:.3rem;display:flex;align-items:center;">✅ Within SLA ${mbInfoIcon(MB_METRIC_TIPS.within)}</div>
                <div style="font-size:1.7rem;font-weight:900;color:#27ae60;line-height:1;">${k.slaWithin}</div>
                <div style="font-size:.75rem;font-weight:700;color:#27ae60;margin-top:.25rem;">${k.slaWithinPct!=null?k.slaWithinPct+'% of open':'—'}</div>
            </div>
            <div onclick="mbToggleSla('outside')" style="cursor:pointer;background:var(--bg-card);border:2px solid ${sf==='outside'?'#e74c3c':'var(--border)'};border-radius:12px;padding:1rem;${sf==='outside'?'background:#e74c3c18;':''}box-shadow:var(--cs);">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:var(--t3);margin-bottom:.3rem;display:flex;align-items:center;">🔴 Outside SLA ${mbInfoIcon(MB_METRIC_TIPS.outside)}</div>
                <div style="font-size:1.7rem;font-weight:900;color:#e74c3c;line-height:1;">${k.slaOutside}</div>
                <div style="font-size:.75rem;font-weight:700;color:#e74c3c;margin-top:.25rem;">${k.slaOutsidePct!=null?k.slaOutsidePct+'% of open':'—'}</div>
            </div>
            <div style="background:var(--bg-card);border:2px solid var(--border);border-radius:12px;padding:1rem;box-shadow:var(--cs);">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:var(--t3);margin-bottom:.3rem;display:flex;align-items:center;">🎯 Compliance ${mbInfoIcon(MB_METRIC_TIPS.compliance)}</div>
                <div style="font-size:1.7rem;font-weight:900;color:${compColor};line-height:1;">${k.slaCompliance!=null?k.slaCompliance+'%':'—'}</div>
                <div style="font-size:.7rem;color:var(--t3);margin-top:.25rem;">within ÷ (within + outside)</div>
            </div>
            <div style="background:var(--bg-card);border:2px solid ${k.dueSoon?'#e67e22':'var(--border)'};border-radius:12px;padding:1rem;box-shadow:var(--cs);">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:var(--t3);margin-bottom:.3rem;display:flex;align-items:center;">⏰ Due in 24h ${mbInfoIcon(MB_METRIC_TIPS.dueSoon)}</div>
...
/* ══════════════════════════════════════════════════════════════
   4b — EMAIL SOURCE & REPEAT THREADS
   ══════════════════════════════════════════════════════════════ */
function mbGetSourceStats(t) {
    t = t || MB.filteredTickets;
    const total = t.length || 1;
    const byType = {};
    t.forEach(x => {
        const label = mbNormSenderType(x.senderType);
    ...
    const types = Object.entries(byType).map(([label, count]) => ({
        ...
        color: mbSenderColor(label),
        icon: mbSenderIcon(label),
    })).sort((a, b) => {
        const ai = MB_SENDER_ORDER.indexOf(a.label), bi = MB_SENDER_ORDER.indexOf(b.label);
        if (ai >= 0 && bi >= 0) return ai - bi;
        if (ai >= 0) return -1;
        if (bi >= 0) return 1;
        return a.label.localeCompare(b.label);
    });
    const repeat = t.filter(x => x.isRepeatSubject).length;
    const repeatThreads = new Set(t.filter(x => x.isRepeatSubject).map(x => x.subjectKey)).size;
    const topRepeats = {};
    t.filter(x => x.isRepeatSubject && x.subjectKey).forEach(x => {
        if (!topRepeats[x.subjectKey]) topRepeats[x.subjectKey] = { key: x.subjectKey, label: x.subject || x.subjectKey, count: x.subjectRepeatCount };
    });
    const topList = Object.values(topRepeats).sort((a, b) => b.count - a.count).slice(0, 8);
    return { total, types, repeat, repeatThreads, topList, repeatPct: Math.round(repeat / total * 100) };
}

function mbRenderSourcePanel() {
    const el = document.getElementById('mb-source-panel');
    if (!el) return;
    const s = mbGetSourceStats();
    const sf = MB.activeSourceFilter;
    const tile = (key, icon, label, count, pct, color, tip) => `
        <div onclick="mbToggleSourceFilter('${String(key).replace(/'/g,"\\'")}')" style="cursor:pointer;background:var(--bg-card);border:2px solid ${sf===key?color:'var(--border)'};border-radius:12px;padding:1rem;${sf===key?`background:${color}18;`:''}box-shadow:var(--cs);">
            <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:var(--t3);margin-bottom:.3rem;display:flex;align-items:center;">${icon} ${label} ${mbInfoIcon(tip)}</div>
            <div style="font-size:1.7rem;font-weight:900;color:${color};line-height:1;">${count}</div>
            <div style="font-size:.75rem;font-weight:700;color:${color};margin-top:.25rem;">${pct}% of total</div>
        </div>`;

    const typeTiles = s.types.map(tp =>
        tile(tp.key, tp.icon, tp.label, tp.count, tp.pct, tp.color, MB_METRIC_TIPS.senderType + ' Value: ' + tp.label)
    ).join('');
    const cols = Math.min(Math.max(s.types.length + 1, 2), 5);

    el.innerHTML = `
    <div class="table-section" style="margin-bottom:1rem;">
        <div class="table-header" style="margin-bottom:.75rem;">
            <h3 class="table-title"><i data-lucide="mail" style="width:18px;height:18px;display:inline-block;vertical-align:middle;margin-right:6px;"></i>Sender Type &amp; Thread Analysis</h3>
            <div style="font-size:12px;color:var(--t3);">From Excel Sender Type column · click to filter${sf?` · <button type="button" class="reset-btn" onclick="mbToggleSourceFilter(null)" style="padding:2px 8px;font-size:11px;">✕ Clear</button>`:''}</div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:.75rem;margin-bottom:.75rem;">
            ${typeTiles}
            ${tile('repeat','🔁','Repeat Threads',s.repeat,s.repeatPct,'#e67e22',MB_METRIC_TIPS.repeat)}
        </div>
        ${s.topList.length ? `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:${MB.showRepeatThreads?'.5rem':'0'};">
            <button type="button" class="export-btn" onclick="MB.showRepeatThreads=!MB.showRepeatThreads;mbRenderSourcePanel();" style="padding:6px 12px;font-size:11px;">
                <i data-lucide="${MB.showRepeatThreads?'chevron-up':'list'}" style="width:13px;height:13px;display:inline-block;vertical-align:middle;margin-right:5px;"></i>
                ${MB.showRepeatThreads?'Hide':'Show'} Top Repeated Threads (${s.repeatThreads} threads)
            </button>
        </div>
        ${MB.showRepeatThreads ? `
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:.5rem;">
            ${s.topList.map(r=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:8px 10px;background:var(--bg-secondary);border-radius:8px;font-size:11px;">
                <span style="color:var(--t2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${String(r.label).replace(/"/g,'&quot;')}">${r.label}</span>
                <span style="color:#e67e22;font-weight:800;white-space:nowrap;">${r.count} emails</span>
            </div>`).join('')}
        </div>` : ''}` : ''}
    </div>`;
    if (typeof lucide!=='undefined') lucide.createIcons();
...
window.mbToggleSourceFilter = function(v) {
    MB.activeSourceFilter = MB.activeSourceFilter === v ? null : v;
    MB.activeKpiFilter = null;
    MB.activeSlaFilter = null;
    mbApplyFilters();
    mbRenderAll();
...
function mbRenderAgeTiles() {
    ...
    const b024 = src.filter(x=>x.ageHours!=null&&x.ageHours<24).length;
    const b2448 = src.filter(x=>x.ageHours!=null&&x.ageHours>=24&&x.ageHours<48).length;
    const b48p = src.filter(x=>x.ageHours!=null&&x.ageHours>=48).length;
    ...
    <div class="table-section" style="margin-bottom:1rem;">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;margin-bottom:1rem;">
            ${[['0-24','0–24 Hours',b024,'#27ae60','✅',MB_METRIC_TIPS.age024],['24-48','24–48 Hours',b2448,'#f39c12','⚠️',MB_METRIC_TIPS.age2448],['48+','48+ Hours',b48p,'#e74c3c','🔴',MB_METRIC_TIPS.age48p]].map(([bucket,label,val,color,icon,tip])=>`
...
/* Chart value labels — only on selected charts (never trend/forecast lines) */
const MB_DATA_LABEL_PLUGIN = {
    id: 'mbDataLabels',
    afterDatasetsDraw(chart) {
        const { ctx, chartArea } = chart;
        if (!chartArea || chart.config.mbSkipLabels) return;
        const isHorizontal = chart.config.options?.indexAxis === 'y';
        const isDoughnut = chart.config.type === 'doughnut' || chart.config.type === 'pie';
        const isLine = chart.config.type === 'line' || chart.data.datasets.some(d => d.type === 'line');
        if (isLine) return;
        const labelCount = chart.data.datasets.reduce((n, ds) => n + (ds.data || []).filter(v => v != null && v !== 0).length, 0);
        if (labelCount > 40) return;
            ...
            meta.data.forEach((element, index) => {
                ...
                if (isHorizontal) {
                    ...
                    if (w < 14) {
                        ctx.fillStyle = '#333';
                        ctx.textAlign = 'left';
                        ctx.fillText(String(val), element.x + 4, element.y);
                    } else {
                        ctx.fillStyle = '#fff';
                        ctx.fillText(String(val), element.x - w / 2, element.y);
                    }
                } else if (isDoughnut) {
                    const total = dataset.data.reduce((a, b) => a + (b || 0), 0);
                    if (total && val / total < 0.06) { ctx.restore(); return; }
                    ctx.fillStyle = '#fff';
                    ...
                    if (h < 14) {
                        ctx.fillStyle = '#333';
                        ctx.fillText(String(val), pos.x, element.y - 6);
                    } else {
                        ctx.fillStyle = '#fff';
                        ctx.fillText(String(val), pos.x, element.y - h / 2);
                    }
                }
                ctx.restore();
            });
...
function mbMakeChart(key, config) {
    ...
    const showLabels = !!config.mbShowLabels;
    const skipLabels = !!config.mbSkipLabels || key === 'trend' || key === 'forecast';
    config.mbSkipLabels = skipLabels;

    if (showLabels && !skipLabels) {
        if (!config.options.layout) config.options.layout = {};
        config.options.layout.padding = Object.assign({ top: 22, right: 12, left: 4, bottom: 4 }, config.options.layout.padding || {});
        if (!config.plugins) config.plugins = [];
        if (!config.plugins.some(p => p && p.id === 'mbDataLabels')) config.plugins.push(MB_DATA_LABEL_PLUGIN);
    }
...
function mbRenderAgeChart() {
    ...
    src.forEach(t=>{
        ...
        if (!smMap[n]) smMap[n]={b024:0,b2448:0,b48p:0};
        if (t.ageHours==null) return;
        if (t.ageHours<24) smMap[n].b024++; else if (t.ageHours<48) smMap[n].b2448++; else smMap[n].b48p++;
    });
    const names = Object.keys(smMap).sort((a,b)=>(smMap[b].b024+smMap[b].b2448+smMap[b].b48p)-(smMap[a].b024+smMap[a].b2448+smMap[a].b48p)).slice(0,15);
    mbMakeChart('age',{
        mbShowLabels: true,
        type:'bar',
        data:{ labels:names, datasets:[
            {label:'0–24 Hours',data:names.map(n=>smMap[n].b024),backgroundColor:'rgba(39,174,96,0.8)',borderRadius:3,borderSkipped:false},
            {label:'24–48 Hours',data:names.map(n=>smMap[n].b2448),backgroundColor:'rgba(243,156,18,0.8)',borderRadius:3,borderSkipped:false},
            {label:'48+ Hours', data:names.map(n=>smMap[n].b48p),backgroundColor:'rgba(231,76,60,0.8)', borderRadius:3,borderSkipped:false},
        ]},
...
function mbBuildAgentStats(list) {
    ...
    list.forEach(t => {
        ...
        if (mbIsOpen(t) && mbIsOutside(t)) s.outside++;
        if (mbIsOpen(t) && mbIsWithin(t)) s.within++;
...
function mbRenderCategories() {
    ...
    mbMakeChart('status',{ mbShowLabels: true, type:'doughnut',
    ...
    mbMakeChart('location',{ mbShowLabels: true, type:'doughnut',
    ...
    const catMap={};
    t.forEach(x=>{ if(!x.category) return; if(!catMap[x.category]) catMap[x.category]={total:0,outside:0}; catMap[x.category].total++; if(mbIsOpen(x)&&mbIsOutside(x)) catMap[x.category].outside++; });
    const sorted=Object.entries(catMap).sort((a,b)=>b[1].total-a[1].total).slice(0,10);
    mbMakeChart('categories',{ mbShowLabels: true, type:'bar',
...
function mbRenderWorkloadChart() {
    ...
    mbMakeChart('workload',{ mbShowLabels: true, type:'bar',
        ...
        options:{responsive:true,maintainAspectRatio:false,
            ...
            scales:{x:{...(ws==='all'?{stacked:true}:{}),grid:{display:false},ticks:{font:{size:10},color:'rgba(160,160,180,0.8)',maxRotation:40}},
                    y:{...(ws==='all'?{stacked:true}:{}),...MB_AX.y, grace:'12%'}}} });
}
...
function mbRenderGrid() {
    ...
    const cols=[
        ...
        mbEnhanceCol({field:'sla',           headerName:'SLA Type',      width:120}),
        ...
        mbEnhanceCol({field:'senderType',    headerName:'Sender Type',   width:110,
         cellRenderer:p=>{ const c=mbSenderColor(p.value); return `<span style="font-size:10px;font-weight:700;color:${c};">${p.value||'—'}</span>`; }}),
        mbEnhanceCol({field:'subject',       headerName:'Subject',       flex:1,tooltipField:'subject',minWidth:220}),
        mbEnhanceCol({field:'subjectRepeatCount',headerName:'Thread #', width:90,
         cellRenderer:p=>p.value>1?`<span style="font-size:10px;font-weight:800;color:#e67e22;">${p.value}</span>`:'—'}),
...
function mbPdfEnsureSpace(pdf, y, need, margin) {
...
function mbPdfHeading(pdf, text, y, margin, size) {
    y = mbPdfEnsureSpace(pdf, y, size * 0.6 + 6, margin);
...
function mbPdfTable(pdf, headers, rows, y, margin, contentW) {
    const colW = contentW / headers.length;
    y = mbPdfEnsureSpace(pdf, y, 8, margin);
    pdf.setFillColor(76, 111, 255);
    pdf.rect(margin, y - 4, contentW, 6, 'F');
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(8);
    pdf.setTextColor(255, 255, 255);
    headers.forEach((h, i) => pdf.text(h, margin + i * colW + 1, y));
    y += 6;
    pdf.setFont('helvetica', 'normal');
    ...
    rows.forEach((row, ri) => {
        ...
        row.forEach((cell, i) => pdf.text(String(cell ?? '—').substring(0, 28), margin + i * colW + 1, y));
        y += 5;
    });
    return y + 4;
}

function mbPdfAddChart(pdf, chartKey, title, y, margin, contentW) {
    const chart = MB.charts[chartKey];
    if (!chart || !chart.canvas) return y;
    try {
        const img = chart.canvas.toDataURL('image/png', 1.0);
        const imgH = 45;
        y = mbPdfHeading(pdf, title, y, margin, 11);
        y = mbPdfEnsureSpace(pdf, y, imgH + 4, margin);
        pdf.addImage(img, 'PNG', margin, y, contentW, imgH);
        return y + imgH + 6;
    } catch (e) { return y; }
}

window.mbExportPDF = async function() {
    if (!MB.dataLoaded) { mbShowToast('No data to export', 'warning'); return; }
    const btn = document.getElementById('mb-export-pdf-btn');
    if (btn) { btn.disabled = true; btn.textContent = 'Generating PDF…'; }
    try {
        mbShowToast('Building PDF report…', 'info');
        await mbLoadPdfLib();
        ...
        const k = mbGetKPIs();
        const src = mbGetSourceStats();
        const agents = mbBuildAgentStats(MB.filteredTickets).sort((a, b) => b.open - a.open).slice(0, 20);
        const dateStr = new Date().toLocaleString('en-GB');

        y = mbPdfHeading(pdf, 'SE Mailbox Intelligence Report', y, margin, 16);
        y = mbPdfLine(pdf, 'Generated: ' + dateStr, y, margin, contentW, 10);
        y = mbPdfLine(pdf, 'Showing ' + MB.filteredTickets.length + ' of ' + MB.allTickets.length + ' emails', y, margin, contentW, 10);

        y = mbPdfHeading(pdf, 'Key Metrics', y, margin, 12);
        y = mbPdfTable(pdf,
            ['Metric', 'Value', 'Detail'],
            [
                ['Total Emails', k.total, 'All in current filter'],
                ['Unassigned', k.unassigned, 'No agent assigned'],
                ['Assigned (Open)', k.inprogress, 'In progress'],
                ['Completed', k.completed, 'Flag completed'],
                ['Avg Age', k.avgAge != null ? k.avgAge.toFixed(1) + ' days' : '—', 'Open emails'],
            ],
            y, margin, contentW);

        y = mbPdfHeading(pdf, 'SLA Performance (Open Emails)', y, margin, 12);
        y = mbPdfTable(pdf,
            ['Metric', 'Count', '%'],
            [
                ['Within SLA', k.slaWithin, (k.slaWithinPct != null ? k.slaWithinPct + '%' : '—')],
                ['Outside SLA', k.slaOutside, (k.slaOutsidePct != null ? k.slaOutsidePct + '%' : '—')],
                ['Compliance', (k.slaCompliance != null ? k.slaCompliance + '%' : '—'), 'Within / (Within+Outside)'],
                ['Due in 24h', k.dueSoon, ''],
            ],
            y, margin, contentW);

        y = mbPdfHeading(pdf, 'Sender Type Breakdown', y, margin, 12);
        y = mbPdfTable(pdf,
            ['Sender Type', 'Count', '% of Total'],
            src.types.map(t => [t.label, t.count, t.pct + '%']).concat([['Repeat Threads', src.repeat, src.repeatPct + '%']]),
            y, margin, contentW);

        if (src.topList.length) {
            y = mbPdfHeading(pdf, 'Top Repeated Subject Threads', y, margin, 12);
            y = mbPdfTable(pdf,
                ['Subject', 'Emails'],
                src.topList.map(r => [r.label, r.count]),
                y, margin, contentW);
        }

        y = mbPdfHeading(pdf, 'Agent Leaderboard (Top 20)', y, margin, 12);
        y = mbPdfTable(pdf,
            ['Agent', 'Open', 'Done', 'Outside', 'SLA %', 'Avg Age'],
            agents.map(a => [
                a.name, a.open, a.completed, a.outside,
                a.slaPct != null ? a.slaPct + '%' : '—',
                a.avgAge != null ? a.avgAge.toFixed(1) + 'd' : '—',
            ]),
            y, margin, contentW);

        const wasHidden = !MB.chartsVisible;
        if (wasHidden) {
            MB.chartsVisible = true;
            const section = document.getElementById('mb-charts-section');
        ...
        pdf.addPage();
        y = margin;
        y = mbPdfHeading(pdf, 'Charts', y, margin, 14);
        y = mbPdfAddChart(pdf, 'workload', 'Agent Workload', y, margin, contentW);
        y = mbPdfAddChart(pdf, 'status', 'Status Split', y, margin, contentW * 0.55);
        y = mbPdfAddChart(pdf, 'categories', 'Top Issue Categories', y, margin, contentW);

        pdf.save('SE_Mailbox_Report_' + new Date().toISOString().slice(0, 10) + '.pdf');

        if (wasHidden) {
            MB.chartsVisible = false;
            const section = document.getElementById('mb-charts-section');
            if (section) section.style.display = 'none';
            mbDestroyCharts();
        }
        mbShowToast('✅ PDF report generated', 'success');
    } catch (e) {
        console.error('[SEMailbox] PDF export failed:', e);
        mbShowToast('❌ PDF export failed — check console', 'error');
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i data-lucide="file-down" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:6px;"></i>Export PDF';
            if (typeof lucide !== 'undefined') lucide.createIcons();
        }
    }
};

function mbHideMailboxRefreshBtn() {
    const section = document.getElementById('seMailboxView');
    if (!section) return;
    section.querySelectorAll('.tab-refresh-btn, button[onclick*="refreshTabData(\'seMailboxView\')"]').forEach(btn => {
...
function mbInjectShell() {
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;margin-bottom:1rem;">
        <div>
            ...
            <div style="font-size:.75rem;color:var(--t3);">Live Outlook · Agent/Citrix assignment · Age &amp; filters based on received date</div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
            <button type="button" class="export-btn" onclick="mbRefreshLive()" style="padding:8px 14px;font-size:12px;">
                <i data-lucide="refresh-cw" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:6px;"></i>Refresh Live
            </button>
            <button type="button" id="mb-export-pdf-btn" class="export-btn" onclick="mbExportPDF()" style="padding:8px 16px;font-size:12px;">
                <i data-lucide="file-down" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:6px;"></i>Export PDF
            </button>
        </div>
    </div>

    <div id="mb-live-bar" style="margin-bottom:12px;padding:10px 14px;background:rgba(76,111,255,0.08);border:1px solid rgba(76,111,255,0.22);border-radius:8px;font-size:12px;"></div>
    ...
    <!-- Email source & repeats -->
    <div id="mb-source-panel"></div>
...
const SEMailbox = {
    async init() {
        try {
            const root = document.getElementById('semailbox-root');
            if (!root) {
                console.error('[SEMailbox] #semailbox-root not found in page.');
                return;
            }
            mbInjectShell();
            mbHideMailboxRefreshBtn();

            const search = document.getElementById('mb-ag-search');
            if (search) search.oninput = e => mbSearch(e.target.value);

            if (typeof lucide !== 'undefined') lucide.createIcons();
            mbUpdateLiveBar();

            if (_mbInited && MB.dataLoaded && !MB.graphError) {
                mbRenderAll();
                mbStartLivePoll();
                return;
            }
            ...
            const loaded = await mbLoadFromOutlook(true);
            if (!loaded && !MB.dataLoaded) {
                root.insertAdjacentHTML('afterbegin',
                    '<div style="padding:12px 14px;margin-bottom:12px;background:rgba(231,76,60,0.08);border:1px solid rgba(231,76,60,0.25);border-radius:8px;font-size:13px;color:#e74c3c;">' +
                    '<b>Live Outlook load failed.</b> Edit <code>MAILBOX_UPN</code> in semailbox-live.js and ensure IT granted <b>Mail.Read.Shared</b> on the mailbox.</div>');
            }
            mbStartLivePoll();
        } catch (err) {
            console.error('[SEMailbox] init failed:', err);
            const root = document.getElementById('semailbox-root');
            if (root) root.innerHTML = '<div style="padding:20px;color:#e74c3c;font-weight:700;">SE Mailbox failed to start: ' + (err.message || err) + '</div>';
        }
    }
};
function initSEMailbox() { return SEMailbox.init(); }

/* Expose globals for SharePoint inline handlers */
if (typeof window !== 'undefined') {
    window.SEMailbox = SEMailbox;
    window.initSEMailbox = initSEMailbox;
    window.mbRefreshLive = mbRefreshLive;
    window.MB = MB;
}
